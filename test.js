...
function testBlur() {
    ...
    document.getElementById('status').innerHTML =
        '<span class="success">✓ Blur test completed - check elements above</span>';
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addEventListener('error', function (e) {
        if (e.message.includes('Content Security Policy') || e.message.includes('CSP')) {
            document.getElementById('status').innerHTML =
                '<span class="error">✗ CSP Violation detected: ' + e.message + '</span>';
        }
    });

    // Initial status
    document.getElementById('status').innerHTML =
        '<span class="success">✓ Test page loaded successfully - no CSP violations detected</span>';
